...
1311	64	77	1328	1344	1248	1522	51	978	1535	1142	390	81	409	68	352";
        ...
        private static int CalculateAnswer1(string input) => CalculateAnswer(input, Problem1Selector);

        private static int CalculateAnswer2(string input) => CalculateAnswer(input, Problem2Selector);

        private static int CalculateAnswer(string input, Func<int[], int> selector)
        {
            return input.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries)
                .Select(line => line.Split('\t', StringSplitOptions.RemoveEmptyEntries))
                .Select(ints => ints.Select(int.Parse).ToArray())
                .Select(selector)
                .Sum();
        }

        private static int Problem1Selector(int[] ints)
        {
            return ints.Max() - ints.Min();
        }

        private static int Problem2Selector(int[] ints)
        {
            var sorted = ints.OrderByDescending(LinqExtensions.Identity<int>()).ToArray();
            ...
            // shouldn't reach this line in theory
            return 0;
        }
    }
}
